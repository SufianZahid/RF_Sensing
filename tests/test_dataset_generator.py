...
def test_dataset_generation_and_nan_free():
    ...
    # Generate dataset of 20 scenarios
...
def test_label_class_balance_tolerances():
    ...
    gen = DatasetGenerator(base_seed=42)
    df = gen.generate_dataset(num_scenarios=100, duration_per_scenario_s=2.0)

    # person_present should be roughly 50/50 (between 35% and 65%)
    present_frac = float(df["person_present"].mean())
    ...
    for noise_val in ["low", "medium", "high"]:
        frac = float((df["noise_level"] == noise_val).mean())
        assert frac >= 0.15, f"Underrepresented noise_level '{noise_val}': {frac:.2f}"

    # movement_state categories (absent: 50%, moving: ~37.5%, stationary: ~12.5%)
    for state_val in ["absent", "stationary", "moving"]:
        frac = float((df["movement_state"] == state_val).mean())
        assert frac >= 0.04, f"Underrepresented movement_state '{state_val}': {frac:.2f}"
...
def test_shortcut_correlation_distance_check():
    ...
    mean_diff = abs(float(np.mean(dist_present)) - float(np.mean(dist_absent)))
    overall_mean = float(np.mean(df["tx_rx_distance"]))
    assert mean_diff / overall_mean < 0.05, f"Spurious distance correlation: diff = {mean_diff:.3f}m"
